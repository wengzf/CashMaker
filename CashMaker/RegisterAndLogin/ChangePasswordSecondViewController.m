//
//  ChangePasswordSecondViewController.m
//  CashMaker
//
...
@interface ChangePasswordSecondViewController()
{
    NSTimer *timer;
    int countdownNumber;
}
...
@implementation ChangePasswordSecondViewController
...
- (IBAction)nextBtnClked:(id)sender {
    ...
    [SMSSDK commitVerificationCode:self.certificateTextField.text phoneNumber:self.telNumber zone:@"86" result:^(NSError *error) {
        ...
        if (!error) {
            ...
            ChangePasswordThirdViewController *vc = [storyboard instantiateViewControllerWithIdentifier:@"ChangePasswordThirdViewController"];
